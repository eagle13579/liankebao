...
# Register
reg = json.dumps({"username": "test_contacts", "password": "Test123456!",
    "name": "测试联系人", "company": "链客宝测试"}).encode()
try:
    req = urllib.request.Request(f"{API}/api/auth/register", data=reg,
...
req = urllib.request.Request(f"{API}/api/contacts/seed", data=seed,
    headers={"Content-Type": "application/json", "Authorization": f"Bearer {token}"})
r = urllib.request.urlopen(req, timeout=5)
d = json.loads(r.read())
print(f"seed结果: {d.get('message', '')}, created: {d.get('data',{}).get('created', 0)}")

# Verify
req2 = urllib.request.Request(f"{API}/api/contacts",
