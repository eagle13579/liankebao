        ...
        <div className="space-y-2 max-h-80 overflow-y-auto">
          {searchResults.map((item) => (
            <div
              ...
              className="p-3 bg-white border border-gray-200 rounded-lg hover:border-brand-300 hover:shadow-sm transition-all cursor-pointer"
            >
              ...
              {item.tags && item.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-1.5">
                  {item.tags.map((tag, i) => (
                    <span key={i} className="px-1.5 py-0.5 text-[10px] bg-brand-50 text-brand-600 rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <input
              ...
              className="w-full px-3 py-1.5 pr-8 text-xs border border-gray-200 rounded-md focus:ring-1 focus:ring-brand-400 focus:border-brand-400"
            />
            {input && (
              <button
                onClick={handleClear}
          ...
          <button
            ...
            className="px-2.5 py-1.5 text-xs font-medium bg-brand-600 text-white rounded-md hover:bg-brand-700 disabled:bg-gray-200 disabled:text-gray-400 transition-colors"
          >
            {loading ? '搜索中...' : '搜索'}
          </button>
        </div>
        ...
        <div>
          <div className="relative">
            <input
              ...
              onKeyDown={handleKeyDown}
              placeholder="描述您需要的合作伙伴..."
              className="w-full pl-10 pr-10 py-3 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-shadow"
            />
            {/* 搜索图标 */}
            <svg
          ...
          <div className="flex gap-2 mt-3">
            <button
              onClick={() => handleSearch()}
              disabled={!input.trim() || loading}
              className="flex-1 py-2 text-sm font-medium rounded-lg text-white bg-brand-600 hover:bg-brand-700 disabled:bg-gray-200 disabled:text-gray-400 transition-colors"
            >
              {loading ? '搜索中...' : '搜索匹配'}
            </button>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {EXAMPLE_QUERIES.slice(0, 2).map((q, i) => (
              <button
                ...
                className="px-3 py-1.5 text-xs rounded-full border border-gray-200 bg-gray-50 text-gray-600 hover:bg-brand-50 hover:border-brand-200 hover:text-brand-600 transition-all"
              >
                "{q}"
              </button>
            ))}
