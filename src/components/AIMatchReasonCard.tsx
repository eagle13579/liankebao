...
function matchScoreGradient(score: number): string {
  ...
  if (score >= 60) return 'from-brand-400 to-cyan-500';
...
export default function AIMatchReasonCard({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/30 bg-white/70 backdrop-blur-xl shadow-lg transition-all duration-300 hover:shadow-xl hover:bg-white/80">
      {/* 毛玻璃装饰光晕 */}
      <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full bg-gradient-to-br from-secondary-200/40 to-brand-200/40 blur-xl pointer-events-none" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {matchReasons.map((reason, i) => (
                <span
                  key={i}
                  className="inline-flex items-center px-2 py-0.5 bg-gradient-to-r from-brand-50 to-secondary-50 text-brand-700 text-[10px] rounded-full border border-brand-100/60 shadow-sm"
                >
                  {reason}
                </span>
              ))}
