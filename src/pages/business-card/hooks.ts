...
export function useBusinessCard() {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    catch(e:any) { setError(e?.message||'scan failed'); } finally { setLoading(false); }
  }, []);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    catch(e:any) { setError(e?.message||'generate failed'); } finally { setLoading(false); }
  }, [fields]);

  const handleMatch = useCallback(async () => {
    if(!cardData) return; setLoading(true);
    try { setMatchResults(await cardApi.matchCard(cardData.id)); setStep('matched'); }
    catch(e:any) { setError(e?.message||'match failed'); } finally { setLoading(false); }
  }, [cardData]);
  ...
  return {
    step,loading,error,fields,suggestions,rawText,cardData,currentPage,totalPages,matchResults,
    showQRModal,qrCodeUrl,commonConnections,copied,fileInputRef,
    handleFileSelect,handleDrop,updateField,handleGenerate,handleMatch,
    handleCopyLink,handleShowQR,handleReset,setCurrentPage,setShowQRModal,
  };
}
