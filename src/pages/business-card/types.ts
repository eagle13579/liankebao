...
export interface AlbumMeta {
  title: string; bio: string; tags: string[];
  theme: "modern" | "classic" | "minimal";
}
export interface CardData {
  id: number; user_id: string; fields: CardFields;
  album_pages: AlbumPage[]; album_meta: AlbumMeta;
  created_at: string;
}
export interface MatchItem {
  id: number; name: string; company: string;
  position: string; match_score: number;
  common_contacts: number; tags: string[];
}
