  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-sm border-b border-gray-100">
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => fileInputRef.current?.click()} className="text-sm text-blue-600 hover:text-blue-700">
              重新上传
            </button>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 pb-8">
        {step === 'upload' && (
          <UploadZone
            loading={loading} dragOver={false}
            onDrop={handleDrop}
        ...
        {step === 'review' && (
          <ReviewForm
            fields={fields} suggestions={suggestions} rawText={rawText}
            onUpdateField={updateField} onGenerate={handleGenerate}
            onReset={handleReset} loading={loading} error={error}
          />
        )}
        {(step === 'preview' || step === 'matched') && cardData && (
          <>
            <FlipBook
              ...
              cardData={cardData} onPageChange={setCurrentPage}
            />
            <CommonConnections count={commonConnections.count} names={commonConnections.names} />
            <ShareActions
              shareUrl={window.location.href} copied={copied}
              matchLoading={loading} onCopy={handleCopyLink}
              onMatch={handleMatch} onShowQR={handleShowQR}
            />
      ...
      <QRCodeModal
        ...
        qrCodeUrl={qrCodeUrl} qrLoading={false}
      />
    </div>
  );
}
