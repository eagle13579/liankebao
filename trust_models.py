...
class TrustQualification(Base):
    ...
    verification_level: Mapped[str] = mapped_column(
        String(16), default=VerificationLevel.AI.value, comment="ai / manual / both"
    )
...
class TrustAuditReport(Base):
    ...
    view_permission: Mapped[str] = mapped_column(
        String(16),
        default=ViewPermission.DIAMOND.value,
        comment="public / gold / diamond / board",
    )
    status: Mapped[str] = mapped_column(
        String(16), default="pending", comment="pending / active / expired / rejected"
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now()
    )
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            if self.audit_period_start
            else None,
            "audit_period_end": self.audit_period_end.isoformat()
            if self.audit_period_end
            else None,
...
class TrustReview(Base):
    ...
    review_text: Mapped[Optional[str]] = mapped_column(Text, comment="评价内容")
    tags: Mapped[Optional[list[str]]] = mapped_column(
        ARRAY(String(32)), comment="评价标签: ['交付准时', '沟通顺畅', '质量可靠']"
    )
    ...
    __table_args__ = (
        UniqueConstraint(
            "from_user_id", "to_user_id", "order_id", name="uq_review_order"
        ),
...
class TrustScoreLog(Base):
    ...
    score_before: Mapped[Optional[float]] = mapped_column(Float, comment="变更前评分")
    score_after: Mapped[Optional[float]] = mapped_column(Float, comment="变更后评分")
