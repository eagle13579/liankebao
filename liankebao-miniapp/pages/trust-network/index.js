...
Page({
  ...
  loadTrust: function() {
    ...
    if (!user) return
    api.request("/api/brochures/" + user.user_id + "/trust_network").then(function(r) {
  ...
  onSearch: function(e) {
    ...
    api.request("/api/users?q=" + q).then(function(r) {
  ...
  addTrust: function(e) {
    ...
    api.request("/api/brochures/" + user.user_id + "/trust_network", "POST", { trusted_user_id: targetId }).then(function(r) {
  ...
  removeTrust: function(e) {
    ...
    wx.showModal({ content: "确定移除信任?", success: function(r) {
      if (r.confirm) api.request("/api/brochures/" + user.user_id + "/trust_network?target_id=" + targetId, "DELETE").then(function() {
