...
def ok(s, m, d=""):
    return {"score": s, "max": m, "detail": d}


def fail(m, d=""):
    return {"score": 0, "max": m, "detail": d}


# ── 1. 数据契约完整性 (0-10) ────────────────────────────────────────
def check_contract(module, url, verbose):
    s, d = 0, []
    for label, ep_suffix, pts in [
        ("字段声明", "/fields", 4),
        ("版本管理", "/versions", 3),
        ("热加载", "/hot-reload", 3),
    ]:
            ...
            r = json.loads(
                urllib.request.urlopen(
                    urllib.request.Request(
                        f"{url}/api/v1/contract/{module}{ep_suffix}", headers={"Accept": "application/json"}
                    ),
                    timeout=TIMEOUT,
                ).read()
            )
            okk = (isinstance(r, list) and len(r) > 0) or (
                isinstance(r, dict) and (r.get("status") == "ok" or r.get("hot_reload"))
            )
            s += pts if okk else 0
    ...
    for label, patterns, pts in [("基础", SQL_BASE, 10), ("变体", SQL_VAR, 10)]:
        ...
        for pat in patterns:
            try:
                ...
                if r.get("blocked") or r.get("status") == "blocked":
                    blocked += 1
            except:
                blocked += 1
        sc = int((blocked / len(patterns)) * pts)
        s += sc
        d.append(f"[{sc}/{pts}] SQL{label}: {blocked}/{len(patterns)}")
    ...
    for label, patterns, pts in [("基础", XSS_BASE, 10), ("变体", XSS_VAR, 10)]:
        ...
        for pat in patterns:
            try:
                ...
                if r.get("blocked") or r.get("sanitized") or r.get("status") == "blocked":
                    blocked += 1
            except:
                blocked += 1
        sc = int((blocked / len(patterns)) * pts)
        s += sc
        d.append(f"[{sc}/{pts}] XSS{label}: {blocked}/{len(patterns)}")
    return ok(s, 20, "; ".join(d) if verbose else "")


# ── 4. 类型混淆防护 (0-10) ─────────────────────────────────────────
TC_PAYLOADS = [
    {"field": "id", "value": "not_an_int"},
    {"field": "amount", "value": {"nested": "object"}},
    {"field": "email", "value": ["list", "instead"]},
    {"field": "active", "value": "not_bool"},
...
def check_typeconf(module, url, verbose):
    ...
    for p in TC_PAYLOADS:
        try:
            ...
            if r.get("valid") is False or r.get("blocked") or r.get("status") in ("blocked", "rejected"):
                blocked += 1
        except:
            blocked += 1
...
def check_ssrf(module, url, verbose):
    ...
    for label, targets, pts in [("元数据", SSRF_META, 5), ("内网IP", SSRF_INT, 5), ("危险协议", SSRF_PROTO, 5)]:
        ...
        sc = int((blocked / len(targets)) * pts)
        s += sc
        d.append(f"[{sc}/{pts}] SSRF{label}: {blocked}/{len(targets)}")
...
def check_quarantine(module, url, verbose):
    ...
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(
                    f"{url}/api/v1/quarantine/ingest",
        ...
        if r.get("quarantined") or r.get("status") == "quarantined":
            s += 5
            d.append("[5/5] 可进入检疫区")
        else:
            d.append("[0/5] 拒绝: " + str(r))
    except Exception as e:
        d.append(f"[0/5] ingest不可达: {e}")
    # 7b isolation
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(f"{url}/api/v1/quarantine/isolation", headers={"Accept": "application/json"}),
                timeout=TIMEOUT,
            ).read()
        )
        if r.get("isolated") or r.get("status") == "isolated":
            s += 10
            d.append("[10/10] 不污染core schema")
        else:
            d.append("[0/10] " + str(r))
    except Exception as e:
        d.append(f"[0/10] isolation不可达: {e}")
    # 7c approve
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(
                    f"{url}/api/v1/quarantine/approve",
        ...
        if r.get("approvable") or r.get("status") in ("pending", "approvable"):
            s += 5
            d.append("[5/5] 审批流程可操作")
        else:
            d.append("[0/5] " + str(r))
    except Exception as e:
        d.append(f"[0/5] approve不可达: {e}")
    return ok(s, 20, "; ".join(d) if verbose else "")
...
def check_audit(module, url, verbose):
    ...
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(f"{url}/api/v1/audit/triggers", headers={"Accept": "application/json"}),
                timeout=TIMEOUT,
            ).read()
        )
        okk = (isinstance(r, list) and len(r) >= 1) or (isinstance(r, dict) and r.get("triggers"))
        s += 5 if okk else 0
        d.append(f"[{5 if okk else 0}/5] 触发器: {'\u2713' if okk else '\u2717'}")
    except Exception as e:
        d.append(f"[0/5] triggers不可达: {e}")
    # 8b immutable
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(f"{url}/api/v1/audit/immutable", headers={"Accept": "application/json"}),
                timeout=TIMEOUT,
            ).read()
        )
        if r.get("immutable") or r.get("status") == "immutable":
            s += 5
            d.append("[5/5] 不可删除")
        else:
            d.append(f"[0/5] 可删除: {r}")
    except Exception as e:
        d.append(f"[0/5] immutable不可达: {e}")
    # 8c trail
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(
                    f"{url}/api/v1/audit/trail?module={module}", headers={"Accept": "application/json"}
                ),
                timeout=TIMEOUT,
            ).read()
        )
        ...
        s += 5 if okk else 0
        d.append(f"[{5 if okk else 0}/5] 可追溯: {'\u2713' if okk else '\u2717'}")
    except Exception as e:
        d.append(f"[0/5] trail不可达: {e}")
    return ok(s, 15, "; ".join(d) if verbose else "")
...
def check_rls(module, url, verbose):
    ...
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(f"{url}/api/v1/rls/self-only", headers={"Accept": "application/json"}),
                timeout=TIMEOUT,
            ).read()
        )
        if r.get("self_only") or r.get("status") == "enforced":
            s += 5
            d.append("[5/5] 用户只能读写自己数据")
        else:
            d.append(f"[0/5] {r}")
    except Exception as e:
        d.append(f"[0/5] self-only不可达: {e}")
    # 9b cross-org
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(
                    f"{url}/api/v1/rls/cross-org",
        ...
        if r.get("blocked") or r.get("visible") is False:
            s += 5
            d.append("[5/5] 跨组织不可见")
        else:
            d.append(f"[0/5] 可见: {r}")
    except Exception as e:
        d.append(f"[0/5] cross-org不可达: {e}")
    # 9c mechanism
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(f"{url}/api/v1/rls/mechanism", headers={"Accept": "application/json"}),
                timeout=TIMEOUT,
            ).read()
        )
        if r.get("mechanism") == "jwt" or r.get("uses_jwt"):
            s += 5
            d.append("[5/5] RLS使用JWT")
        else:
            d.append(f"[0/5] 非JWT: {r}")
    except Exception as e:
        d.append(f"[0/5] mechanism不可达: {e}")
    return ok(s, 15, "; ".join(d) if verbose else "")
...
def check_dwg(module, url, verbose):
    ...
    for label, ep, pts in [("熔断", "circuit-breaker", 5), ("降级", "degraded", 5), ("白名单", "whitelist", 5)]:
        try:
            r = json.loads(
                urllib.request.urlopen(
                    urllib.request.Request(f"{url}/api/v1/dwg/{ep}", headers={"Accept": "application/json"}),
                    timeout=TIMEOUT,
                ).read()
            )
            okk = (isinstance(r, list) and len(r) >= 1) or (
                isinstance(r, dict)
                and (
                    r.get("enabled")
                    or r.get("status") in ("open", "closed", "half-open")
                    or r.get("degraded_path")
                    or r.get("fallback")
                    or r.get("whitelist")
                )
            )
...
def check_anomaly(module, url, verbose):
    ...
    for label, ep, pts in [("统计基线", "baseline", 5), ("冷启动降级", "cold-start", 5)]:
        try:
            r = json.loads(
                urllib.request.urlopen(
                    urllib.request.Request(f"{url}/api/v1/anomaly/{ep}", headers={"Accept": "application/json"}),
                    timeout=TIMEOUT,
                ).read()
            )
...
def check_zhanlang(module, url, verbose):
    try:
        r = json.loads(
            urllib.request.urlopen(
                urllib.request.Request(
                    f"{url}/api/v1/zhanlang/results?module={module}", headers={"Accept": "application/json"}
                ),
                timeout=TIMEOUT,
            ).read()
        )
    except Exception as e:
        return fail(10, f"端点不可达: {e}")
    total = len(r) if isinstance(r, list) else r.get("total", r.get("count", 0))
    passed = (
        sum(1 for x in r if x.get("passed") or x.get("status") == "pass")
        if isinstance(r, list)
        else r.get("passed", r.get("success", 0))
    )
    s = 0
    d = []
    s += 5 if total >= 20 else 0
    d.append(f"[{5 if total >= 20 else 0}/5] 攻击完成: {total}/20")
    pct = (passed / total * 100) if total > 0 else 0
    s += 5 if pct >= 80 else 0
    d.append(f"[{5 if pct >= 80 else 0}/5] 通过率: {pct:.0f}%")
    return ok(s, 10, "; ".join(d) if verbose else "")
...
def check_expediency(module, url, verbose):
    ...
    for label, ep, pts in [("紧急绕过", "bypass", 3), ("灰度发布", "canary", 3), ("cronjob", "cronjob", 4)]:
        try:
            r = json.loads(
                urllib.request.urlopen(
                    urllib.request.Request(f"{url}/api/v1/expediency/{ep}", headers={"Accept": "application/json"}),
                    timeout=TIMEOUT,
                ).read()
            )
            okk = any(
                r.get(k) for k in ("bypass_available", "enabled", "canary", "gray_release", "cronjob", "scheduled")
            )
            s += pts if okk else 0
            d.append(f"[+{pts if okk else 0}] {label}: {'\u2713' if okk else '\u2717'}")
        except Exception as e:
            d.append(f"[0] {label}不可达: {e}")
    return ok(s, 10, "; ".join(d) if verbose else "")


# ── Check Registry ────────────────────────────────────────────────
CHECKS = [
    ("1.  数据契约完整性", check_contract, 10),
    ("2.  SQL注入防护", check_sql, 20),
    ("3.  XSS防护", check_xss, 20),
    ("4.  类型混淆防护", check_typeconf, 10),
    ("5.  Unicode攻击防护", check_unicode, 10),
    ("6.  SSRF防护", check_ssrf, 15),
    ("7.  检疫区流程", check_quarantine, 20),
    ("8.  审计", check_audit, 15),
    ("9.  RLS行级安全", check_rls, 15),
    ("10. DWG熔断/降级", check_dwg, 15),
    ("11. 异常评分引擎", check_anomaly, 10),
    ("12. 战狼攻击", check_zhanlang, 10),
...
def main():
    ap = argparse.ArgumentParser(
        description="Gate 3 数据安全门 CLI 验证器 (量化评分版)",
    ...
    if args.all:
        for mod, url in [
            ("ai_card", "http://localhost:8001"),
            ("chainke", "http://localhost:8000"),
            ("data_pipeline", "http://localhost:8002"),
        ]:
            ...
            if args.json:
                    ...
                    json.dumps(
                        {
                            ...
                            "passed": total >= PASS_THRESHOLD,
                            "checks": [
                                {"name": n, "score": r["score"], "max": r["max"], "detail": r.get("detail", "")}
                                for n, r, _ in rs
                            ],
                        },
                        ensure_ascii=False,
                ...
                print_report(rs, total, bonus, mod, args.verbose)
        return

    if not args.module or not args.url:
        print(f"{R}错误: 使用 --module/-m 和 --url/-u 指定目标模块, 或 --all 全扫描{RS}")
        sys.exit(1)
    ...
    if args.json:
        out = json.dumps(
            {
                ...
                "passed": total >= PASS_THRESHOLD,
                "checks": [
                    {"name": n, "score": r["score"], "max": r["max"], "detail": r.get("detail", "")} for n, r, _ in rs
                ],
            },
            ensure_ascii=False,
            indent=2,
        ...
        if args.output:
            ...
            print_report(rs, total, bonus, args.module, args.verbose)
            sys.stdout = _out
            open(args.output, "w", encoding="utf-8").write(buf.getvalue())
