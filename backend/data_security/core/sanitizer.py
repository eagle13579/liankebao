...
def _detect_json_injection(text: str) -> Optional[str]:
    """检测JSON注入/原型污染"""
...
def _detect_ssrf(text: str) -> Optional[str]:
    """检测SSRF攻击向量"""
...
class Sanitizer:
    ...
    def sanitize_string(
        self, text: str, field_name: str = "<unknown>",
    ) -> Tuple[str, List[str]]:
        """
        对单个字符串执行全量消毒工序
        ...
        if len(text) < before:
            warnings.append(
                f"[{field_name}] 移除了 {before - len(text)} 个控制字符"
            )
        ...
        if len(text) < before:
            warnings.append(
                f"[{field_name}] 移除了 {before - len(text)} 个零宽字符"
            )
        ...
        if len(text) < before:
            warnings.append(
                f"[{field_name}] 移除了 {before - len(text)} 个双向文本覆盖字符"
            )
        ...
        if homoglyphs:
            warnings.append(
                f"[{field_name}] 检测到 {len(homoglyphs)} 个西里尔同形字母: "
                f"{'; '.join(homoglyphs[:5])}"
            )
        ...
        if len(text) > self.max_string_length:
            warnings.append(
                f"[{field_name}] 字符串长度 {len(text)} 超过限制 "
                f"{self.max_string_length}，截断处理"
            )
    ...
    def _check_depth(self, depth: int) -> None:
        ...
        if depth > self.max_depth:
            raise MaxDepthExceededError(
                f"超过最大嵌套深度 {self.max_depth} (当前深度 {depth})"
            )
    ...
    def _check_keys(self, obj: Dict[str, Any], field_name: str) -> List[str]:
        ...
        if len(obj) > self.max_keys:
            return [
                f"[{field_name}] 键数量 {len(obj)} 超过限制 {self.max_keys}"
            ]
        return []
    ...
    def _check_list_length(self, lst: List[Any], field_name: str) -> List[str]:
        ...
        if len(lst) > self.max_list_length:
            return [
                f"[{field_name}] 列表长度 {len(lst)} 超过限制 {self.max_list_length}"
            ]
        return []
    ...
    def sanitize(
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                safe_key, _ = self.sanitize_string(
                    str(key), field_name=f"{field_name}.key"
                )
    ...
    def _sanitize_recursive(
        ...
        if isinstance(data, dict):
            ...
            result: Dict[str, Any] = {}
            for key, value in data.items():
                safe_key, key_warns = self.sanitize_string(
                    str(key), field_name=f"{field_name}.key"
                )
...
def _demo():
    ...
    for name, value in test_cases:
        ...
        if result.get('injection_detected'):
            print(f"  !! 检测到攻击: pattern={result['pattern']}, "
