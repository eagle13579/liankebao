...
from recharge.models import BalanceLog, RechargeOrder, UserBalance
        ...
        if openid and config.is_configured:
            try:
                ...
                result = await wxpay.create_jsapi_order(
                    ...
                    description=f"链客宝AI充值-{req.amount:.2f}元",
...
def list_recharge_orders(
    ...
    """用户充值记录列表（分页）"""
    query = db.query(RechargeOrder).filter(RechargeOrder.user_id == current_user.id)

    total = query.count()
    orders = query.order_by(desc(RechargeOrder.created_at)).offset((page - 1) * limit).limit(limit).all()
...
def list_balance_logs(
    ...
    """分页查询余额流水记录"""
    query = db.query(BalanceLog).filter(BalanceLog.user_id == current_user.id)

    total = query.count()
    logs = query.order_by(desc(BalanceLog.created_at)).offset((page - 1) * limit).limit(limit).all()
