...
class AIChatbotEngine:
    ...
    # 会话管理
    # ----------------------------------------------------------
    def get_or_create_session(self, session_id: str | None = None, user_id: int | None = None) -> ChatSession:
        """获取或创建聊天会话
    ...
    def process_message(
        ...
        try:
            # ---- 1. 获取/创建会话 ----
            session = self.get_or_create_session(session_id=session_id, user_id=user_id)
    ...
    # 转人工
    # ----------------------------------------------------------
    def _handle_escalation(self, session: ChatSession, text: str) -> dict[str, Any]:
        """处理转人工请求
        ...
        ticket_id = f"ESC-{datetime.now(UTC).strftime('%Y%m%d')}-{self._escalation_counter:04d}"
        ...
        bot_msg = ChatMessage(
            role=MessageRole.BOT,
            content=(f"【系统】已创建转人工工单 {ticket_id}，客服人员将尽快与您联系。"),
