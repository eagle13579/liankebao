...
迁移说明:
  源项目: D:\链客宝\backend\app\middleware\api_key_auth.py
  适配: 使用 chainke-full 的现有 AuthMiddleware (request.state.user) 模式
  模型: ApiKey, ApiUsageLog 位于 app.models.developer_portal
"""
...
from app.models.developer_portal_models import ApiKey, ApiUsageLog
...
async def api_key_middleware(request: Request, call_next: Callable):
    ...
    与 chainke-full AuthMiddleware 配合:
      - AuthMiddleware 先验证 JWT (request.state.user)
      - api_key_middleware 验证 X-API-Key (request.state.api_key_info)
      - 两者互不干扰，开发者端点优先使用 API Key 认证
    """
