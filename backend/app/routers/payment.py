...
class WxPayUnifiedOrderRequest(BaseModel):
    """微信统一下单请求"""
    order_id: int
    openid: Optional[str] = None  # 兼容已有逻辑：使用用户 wechat_openid
...
async def wxpay_callback(
    ...
    if is_v3 and has_config(PLATFORM_WXPAY):
        ...
        try:
            ...
            out_trade_no = notify_data.get("out_trade_no", "")
            transaction_id = notify_data.get("transaction_id", "") or notify_data.get("wx_transaction_id", f"mock_tx_{int(time.time())}")
