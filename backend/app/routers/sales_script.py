...
class SalesScript(BaseModel):
    ...
    tension_score: Optional[int] = None  # 张力评分 0-100
    tags: list[str] = []
    created_at: str = ""
    updated_at: str = ""
...
TENSION_WEAPONS = {
    ...
    "magic_words": {
        ...
        "specificity": {
            "name": "具象化引导词",
            "words": ["具体来说", "例如", "实际上", "数字上看", "换句话说"]
        },
        "contrast": {
            "name": "对比引导词",
            "words": ["相比之下", "传统方式", "而我们的", "别人还在", "我们已经"]
        },
        "future_pacing": {
            "name": "未来投射引导词",
...
try:
    ...
    async def get_data_augmenter(mode: Optional[str] = None):
    ...
    async def get_magic_words(category: Optional[str] = None):
        """话术引导词推荐：按分类筛选"""
    ...
    async def get_tension_check(score: Optional[int] = None):
        """张力自检评分：给定分数返回对应等级建议"""
    ...
    print(f"[ABACC] 张力武器库: 数据增强器3模式 + 引导词5分类 + 自检3阶")
