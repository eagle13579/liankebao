...
class Hypothesis(BaseModel):
    ...
    risk_score: Optional[int] = None  # 1-10 验证不通过的风险
...
def _find_hypothesis(hypothesis_id: int) -> Optional[Hypothesis]:
    for h in HYPOTHESES:
...
try:
    ...
    async def list_hypotheses(category: Optional[str] = None, status: Optional[str] = None):
        """按分类和状态筛选假设列表"""
        results = HYPOTHESES
        if category:
    ...
    async def list_experiments(hypothesis_id: Optional[int] = None):
        if hypothesis_id:
