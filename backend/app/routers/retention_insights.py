...
ACTIVITIES: list[UserActivity] = [
    UserActivity(
        ...
        actions=24,
        is_active=True,
        last_active_at="2026-06-20T15:30:00Z",
    ),
    UserActivity(
        id=2,
        user_id="U002",
    ...
    UserActivity(
        ...
        user_id="U003",
        username="王芳",
        cohort_period="2026-04",
        activity_period="2026-06",
        actions=42,
        is_active=True,
        last_active_at="2026-06-21T11:15:00Z",
    ),
    UserActivity(
        id=4,
        user_id="U004",
        ...
        actions=0,
        is_active=False,
        last_active_at="2026-04-28T16:45:00Z",
    ),
    UserActivity(
        id=5,
        user_id="U005",
        username="陈静",
        cohort_period="2026-05",
        activity_period="2026-06",
        actions=18,
        is_active=True,
        last_active_at="2026-06-19T14:00:00Z",
    ),
]
...
CHURN_SIGNALS: list[ChurnSignal] = [
    ChurnSignal(
        ...
        severity="高",
        description="超过30天未活跃，近期0行为",
        detected_at="2026-06-15T08:00:00Z",
        days_since_last_active=42,
        recommended_action="发送召回推送+专属优惠券",
        resolved=False,
    ),
    ChurnSignal(
        id=2,
        ...
        signal_type="inactivity",
        severity="高",
        description="超过45天未活跃，用户可能完全流失",
        detected_at="2026-06-15T08:00:00Z",
        days_since_last_active=53,
        recommended_action="AI电话召回+1对1客服回访",
        resolved=False,
    ),
    ChurnSignal(
        id=3,
        ...
        severity="低",
        description="上月活跃度下降40%（上月40次->本月24次）",
        detected_at="2026-06-20T16:00:00Z",
        days_since_last_active=5,
        recommended_action="推送新功能更新通知",
        resolved=False,
    ),
]
...
def _find_cohort(cohort_id: int) -> Cohort | None:
    """按ID查找Cohort"""
    for c in COHORTS:
        if c.id == cohort_id:
            return c
...
try:
    ...
    async def get_retention_matrix():
        ...
        for cohort in COHORTS:
            ...
            matrix.append({"cohort": cohort, "retention": retentions})
        return {"matrix": matrix}

    # === 用户活跃度 ===

    @router.get("/activities", summary="获取用户活跃记录")
    async def list_activities(cohort_period: str | None = None, active_only: bool | None = None):
        results = ACTIVITIES
        if cohort_period:
    ...
    async def list_churn_signals(severity: str | None = None, resolved: bool | None = None):
    ...
    async def list_strategies(status: str | None = None, priority: str | None = None):
    ...
    print(
        f"[M7] Cohort: {len(COHORTS)} 个 | 留存数据点: {len(COHORT_RETENTION)} 个 | 流失信号: {len(CHURN_SIGNALS)} 个"
    )
