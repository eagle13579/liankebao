...
COST_ENTRIES: list[CostEntry] = [
    ...
    CostEntry(
        ...
        amount=12000.0,
        period="2026-06",
        description="3家渠道伙伴15%分成",
    ),
    CostEntry(
        id=5, name="外呼系统月费", category="工具订阅", amount=3000.0, period="2026-06", description="AI外呼系统月费"
    ),
]
...
SNAPSHOTS: list[UnitEconomicsSnapshot] = [
    UnitEconomicsSnapshot(
        ...
        ltv=42560.0,
        ltv_cac_ratio=2.96,
        avg_revenue_per_customer=8520.0,
...
try:
    ...
    async def list_costs(period: str | None = None, category: str | None = None):
    ...
    async def list_revenues(period: str | None = None, channel: str | None = None):
    ...
    async def get_dashboard(period: str | None = None):
        """返回核心指标总览"""
    ...
    async def get_channel_economics(period: str | None = None):
    ...
    print(
        f"[M6] 成本条目: {len(COST_ENTRIES)} 条 | 收入条目: {len(REVENUE_ENTRIES)} 条 | 渠道: {len(CHANNEL_ECONOMICS)} 个"
    )
