...
class RetroItem(BaseModel):
    ...
    board_id: int
    stage: (
        str  # F1(目标回顾)/F2(结果评估)/F3(亮点发现)/F4(问题分析)/F5(根因挖掘)/F6(经验提炼)/F7(规律总结)/F8(行动规划)
    )
...
RETRO_ITEMS: list[RetroItem] = [
    RetroItem(
        ...
        stage="F3",
        category="亮点",
        content="AI匹配推荐功能上线后获客转化率提升28%，超出预期目标15%",
        author="张明",
        priority="高",
    ),
    RetroItem(
    ...
    RetroItem(
        ...
        stage="F4",
        category="问题",
        content="企业版上线后首周发现3个P0级bug，影响用户体验",
        author="赵雷",
        priority="高",
    ),
    RetroItem(
        ...
        stage="F4",
        category="问题",
        content="电销团队话术培训不足，新人转化率仅为老员工40%",
        author="李华",
        priority="中",
    ),
    RetroItem(
        ...
        stage="F4",
        category="问题",
        content="API文档更新滞后，导致2家合作伙伴集成延期",
        author="陈睿",
        priority="中",
    ),
    RetroItem(
        ...
        stage="F1",
        category="目标",
        content="Q2目标: MAU达到5000，营收突破80万",
        author="张明",
        priority="高",
    ),
    RetroItem(
        ...
        stage="F2",
        category="结果",
        content="MAU达到4300(完成86%)，营收72万(完成90%)",
        author="张明",
        priority="高",
    ),
]
...
ACTION_ITEMS: list[ActionItem] = [
    ActionItem(
        id=1,
        board_id=1,
        title="修复P0级bug并补充测试用例",
        description="企业版3个P0 bug已在修复中，需补充自动化测试防止回归",
        owner="赵雷",
        deadline="2026-06-25",
        priority="高",
        status="进行中",
    ...
    ActionItem(
        ...
        board_id=1,
        title="制定电销新人培训SOP",
        description="编写标准话术模板+模拟演练+首月师徒制",
        owner="李华",
        deadline="2026-07-05",
        priority="高",
        status="待开始",
    ...
    ActionItem(
        ...
        board_id=1,
        title="API文档规范化",
        description="建立API文档审查流程，每次发布前强制更新",
        owner="陈睿",
        deadline="2026-06-30",
        priority="中",
        status="待开始",
...
try:
    ...
    async def list_boards(status: str | None = None, project: str | None = None):
        results = BOARDS
        if status:
    ...
    async def list_items(board_id: int, stage: str | None = None, category: str | None = None):
    ...
    async def list_actions(board_id: int, status: str | None = None, owner: str | None = None):
