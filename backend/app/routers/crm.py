...
from app.models import Deal, DealActivity, User
from app.schemas import DealActivityCreate, DealCreate, DealUpdate
...
def list_deals(
    ...
    q = db.query(Deal)
    if stage:
        q = q.filter(Deal.stage == _normalize_stage(stage))
    if search:
...
def get_deal(deal_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    activities = (
        db.query(DealActivity).filter(DealActivity.deal_id == deal_id).order_by(DealActivity.created_at.desc()).all()
    )
...
def update_deal(
    deal_id: int, data: DealUpdate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)
):
    deal = db.query(Deal).filter(Deal.id == deal_id).first()
    if not deal:
        raise HTTPException(404, "商机不存在")
...
def create_deal_activity(
    ...
    if not deal:
        raise HTTPException(404, "商机不存在")
    act = DealActivity(
        deal_id=deal.id, user_id=current_user.id, action_type=data.action_type, summary=data.summary, detail=data.detail
    )
    db.add(act)
    db.commit()
