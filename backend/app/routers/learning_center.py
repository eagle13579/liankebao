...
class LearningProgress(BaseModel):
    ...
    quiz_score: Optional[float] = None  # 测验得分 0-100
...
COURSES: list[Course] = [
    Course(
        id=1, title="B2B销售实战：从陌拜到成交", description="系统化掌握B2B销售全流程，包含ABACC话术框架、客户心理学、谈判技巧等核心能力",
        category="销售技巧", level="中级", duration_minutes=480, modules_count=10,
        instructor="陈睿", tags=["B2B", "销售", "话术", "ABACC"], rating=4.7,
        enrolled_count=128, completion_rate=0.68, status="上架",
        created_at="2026-05-01T08:00:00Z"
    ),
    Course(
        id=2, title="链客宝产品专家认证", description="全面掌握链客宝所有功能模块，成为产品专家并获得官方认证",
        category="产品使用", level="初级", duration_minutes=360, modules_count=8,
        instructor="王芳", tags=["产品", "认证", "入门"], rating=4.5,
        enrolled_count=256, completion_rate=0.45, status="上架",
        created_at="2026-05-15T10:00:00Z"
    ),
    Course(
        id=3, title="AI获客引擎深度训练", description="深入理解AI供需匹配原理，掌握数据驱动获客方法",
        category="数据分析", level="高级", duration_minutes=600, modules_count=12,
        instructor="张明", tags=["AI", "获客", "数据分析", "高级"], rating=4.9,
        enrolled_count=67, completion_rate=0.32, status="上架",
        created_at="2026-06-01T09:00:00Z"
    ),
]

MODULES: list[Module] = [
    Module(id=1, course_id=1, module_code="X1", title="AI导师路径规划", description="AI根据您的销售经验和目标定制学习路径", content_type="interactive", order=1),
    Module(id=2, course_id=1, module_code="X2", title="销售能力诊断", description="30分钟前置测评，评估您的销售能力基线", content_type="quiz", order=2),
    Module(id=3, course_id=1, module_code="X3", title="ABACC五步说服框架", description="深入掌握注意→痛点→改变→好奇→行动的完整话术框架", content_type="video", order=3),
    Module(id=4, course_id=1, module_code="X4", title="B2B成交案例拆解", description="分析5个真实B2B成交案例，理解ABACC的实际应用", content_type="case", order=4),
    Module(id=5, course_id=1, module_code="X5", title="模拟客户对话", description="AI模拟客户角色，进行真实销售对话练习", content_type="interactive", order=5),
    Module(id=6, course_id=2, module_code="X1", title="产品全景认知", description="链客宝产品体系总览：获客→匹配→转化→服务全链路", content_type="video", order=1),
    Module(id=7, course_id=2, module_code="X3", title="核心功能实操", description="手把手教学：AI名片、供需匹配、人脉管理、数据看板", content_type="video", order=2),
    Module(id=8, course_id=3, module_code="X3", title="AI匹配算法原理", description="深入理解协同过滤+知识图谱的匹配机制", content_type="video", order=1),
]

LESSONS: list[Lesson] = [
    Lesson(id=1, module_id=3, title="Attention：3秒建立信任的开场", content="在B2B销售中，第一句话决定了客户是否愿意继续听下去。本课教你如何用'行业关键词+具体数据+价值承诺'的结构在3秒内抓住客户注意力。", content_type="video", order=1, duration_minutes=15),
    Lesson(id=2, module_id=3, title="Before：让客户自己说'痛'", content="不是告诉客户他们有问题，而是引导客户自己意识到问题的存在。学会用SCQA模型构建痛点对话。", content_type="video", order=2, duration_minutes=20),
    Lesson(id=3, module_id=3, title="After：制造购买后的画面感", content="人们买的不是产品，是更好的自己。学会用'当您使用...之后'句式帮客户构建购买后的理想画面。", content_type="video", order=3, duration_minutes=18),
    Lesson(id=4, module_id=3, title="Curiosity：唯一性才是成交开关", content="客户不关心你有多好，只关心你有多不同。掌握差异化卖点的3个提炼方法和话术转化技巧。", content_type="video", order=4, duration_minutes=22),
    Lesson(id=5, module_id=3, title="Call Action：让说'好'比说'不'更容易", content="好的行动号召是让客户的下一个动作变得极其简单。二选一法则+紧迫感构建+零风险承诺。", content_type="video", order=5, duration_minutes=15),
]

PROGRESSES: list[LearningProgress] = [
    LearningProgress(id=1, user_id="U001", course_id=1, progress_pct=65.0, completed_lessons=13, total_lessons=20, time_spent_minutes=320, status="学习中", started_at="2026-06-10T10:00:00Z"),
    LearningProgress(id=2, user_id="U001", course_id=2, progress_pct=100.0, completed_lessons=16, total_lessons=16, time_spent_minutes=280, quiz_score=92.0, status="已完成", started_at="2026-05-20T09:00:00Z", completed_at="2026-06-05T16:00:00Z"),
    LearningProgress(id=3, user_id="U003", course_id=1, progress_pct=25.0, completed_lessons=5, total_lessons=20, time_spent_minutes=90, status="学习中", started_at="2026-06-18T14:00:00Z"),
]

AI_TUTOR_MESSAGES: list[AiTutorMessage] = []

CERTIFICATIONS: list[Certification] = [
    Certification(id=1, user_id="U001", user_name="张明", course_id=2, course_title="链客宝产品专家认证", score=92.0, passed=True, skills=["产品操作", "客户配置", "数据分析"]),
]
...
def _suggest_next_module(target) -> Optional[dict]:
    """根据学习进度建议下一个模块"""
...
def _filter_courses(category: Optional[str], level: Optional[str], status: Optional[str]) -> list:
    """按筛选条件过滤课程"""
    results = COURSES
    if category:
...
try:
    ...
    async def list_courses(category: Optional[str] = None, level: Optional[str] = None, status: Optional[str] = None):
    ...
    async def list_modules(course_id: int):
        ...
        # 按X1-X10顺序排序
        results = sorted(results, key=lambda m: LEARNING_PATH.index(next(p for p in LEARNING_PATH if p["code"] == m.module_code)) if any(p["code"] == m.module_code for p in LEARNING_PATH) else 99)
    ...
    async def get_learning_dashboard(user_id: str):
        ...
        avg_completion = round(sum(p.progress_pct for p in user_progresses) / total_courses, 1) if total_courses > 0 else 0.0
    ...
    def _get_next_learning_stage(user_id: str) -> Optional[dict]:
        """建议用户下一个学习阶段"""
