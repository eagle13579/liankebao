...
class KnowledgeAgent(BaseAgent):
    ...
    async def generate_docs(self, code_path: Any) -> dict[str, Any]:
        ...
        for func_name, params_str, return_type, func_doc in functions:
            if not func_name.startswith("_"):
                # Parse parameters
                params = [
                    p.strip().split(":")[0].strip() for p in params_str.split(",") if p.strip() and "self" not in p
                ]
                func_docs.append(
                    {
                        "name": func_name,
        ...
        doc_lines.append("---")
        doc_lines.append(f"*Auto-generated by KnowledgeAgent on {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}*")
        doc_lines.append("")
        ...
        await self.learn(
            observation=(
                f"Generated documentation for {path}: {len(class_docs)} classes, {len(func_docs)} functions documented"
            ),
            metadata={
                "code_path": path,
    ...
    async def create_adr(self, title: Any, decision: str = "", context: str = "") -> dict[str, Any]:
        ...
        adr_body = (
            ...
            f"---\n"
            f"*Created by KnowledgeAgent on {datetime.now(UTC).strftime('%Y-%m-%d %H:%M UTC')}*\n"
        )
        ...
        await self.learn(
            observation=(
                f"ADR-{adr_number:03d}: {title_str} — {status}. Context: {context[:100] if context else 'Not provided'}"
            ),
            metadata={
                "adr_number": adr_number,
                "title": title_str,
    ...
    async def _handle_deploy_production(self, event: Any) -> None:
        ...
        version = payload.get("version", payload.get("tag", f"v{datetime.now(UTC).strftime('%Y.%m.%d.%H%M')}"))
