...
down_revision: Union[str, Sequence[str], None] = 'c0398919c9e1'
...
def upgrade() -> None:
    ...
    op.create_foreign_key(
        'fk_products_organization', 'products', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_orders_organization', 'orders', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_withdrawals_organization', 'withdrawals', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_contacts_organization', 'contacts', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_activities_organization', 'activities', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_import_history_organization', 'import_history', 'organizations',
        ['organization_id'], ['id'],
    )
    ...
    op.create_foreign_key(
        'fk_business_needs_organization', 'business_needs', 'organizations',
        ['organization_id'], ['id'],
    )
...
def downgrade() -> None:
    ...
    # 按依赖顺序反向操作
    tables_with_org = [
        'business_needs', 'import_history', 'activities',
        'contacts', 'withdrawals', 'orders', 'products', 'users',
    ]

    for table in tables_with_org:
        op.drop_constraint(
            f'fk_{table}_organization', table, type_='foreignkey',
        )
