...
# ── 链客宝AI对接配置（实时读取环境变量或使用默认值） ────
CHAINKE_API_BASE = os.environ.get("CHAINKE_API_BASE", "") or ""
"""链客宝AI用户系统对接基地址。设为空字符串则使用本地 SQLite 用户系统（默认）。
Phase 3: 信任网络→企盟匹配桥接，可指向链客宝AI网关 :5136。"""
...
def _get_chainke_bridge():
    """延迟加载 chainke_bridge 模块，避免启动时依赖链客宝AI后端。"""
    ...
    if _chainke_bridge is None:
        try:
            ...
            _chainke_bridge = cb
            if CHAINKE_API_BASE:
                logger.info("链客宝AI桥接模块已加载: %s", CHAINKE_API_BASE)
            else:
                logger.info("链客宝AI桥接模块已加载（本地模式，未配置远程地址）")
        except ImportError:
            logger.warning("chainke_bridge 模块未找到，链客宝AI同步功能不可用")
            _chainke_bridge = None
...
def db_get_brochure(brochure_id: str) -> dict | None:
    """从 SQLite 获取单个画册。"""
    conn = get_db_conn()
    try:
        row = conn.execute("SELECT data FROM brochures WHERE brochure_id = ?", (brochure_id,)).fetchone()
        if row:
...
def db_create_user(name: str, phone: str, password: str) -> dict | None:
    """创建新用户。返回用户信息或 None（手机号已存在）。"""
    conn = get_db_conn()
    try:
        # 检查手机号是否已注册
        existing = conn.execute("SELECT user_id FROM auth_users WHERE phone = ?", (phone,)).fetchone()
        if existing:
            return None
...
def db_get_user_by_phone(phone: str) -> dict | None:
    """通过手机号查找用户。"""
    conn = get_db_conn()
    try:
        row = conn.execute("SELECT * FROM auth_users WHERE phone = ?", (phone,)).fetchone()
        return dict(row) if row else None
    finally:
        conn.close()
...
def db_authenticate_user(phone: str, password: str) -> dict | None:
...
def db_get_user_brochure_ids(user_id: str) -> list[str]:
    ...
    conn = get_db_conn()
    try:
        rows = conn.execute("SELECT brochure_id FROM brochures WHERE user_id = ?", (user_id,)).fetchall()
...
def db_upsert_brochure(brochure_id: str, user_id: str, data: dict) -> bool:
    ...
    try:
        ...
        conn.commit()
        # 写入后验证
        verify = conn.execute("SELECT brochure_id FROM brochures WHERE brochure_id = ?", (brochure_id,)).fetchone()
...
def db_log_visitor(brochure_id: str, visitor_ip: str = "", visitor_name: str = "", source: str = "direct") -> bool:
    """记录访客访问。"""
    conn = get_db_conn()
    try:
        conn.execute(
...
class BrochureUpdate(BaseModel):
    ...
    trust_network: list[str] | None = None
...
async def http_exception_handler(request: Request, exc: Exception):
    ...
    logger.error(
        "请求异常: path=%s, trace_id=%s, status=%d, detail=%s", request.url.path, trace_id, status_code, detail
    )
...
def index(request: Request):
    ...
    templates = Jinja2Templates(
        directory=os.path.join(os.path.dirname(os.path.abspath(__file__)), "backend", "app", "templates")
    )
...
def update_brochure(user_id: str, data: BrochureUpdate, current_user_id: str = Depends(require_user_id)):
    bid = ensure_brochure_id(user_id)
    brochure = BROCHURES.get(bid)
    if not brochure:
        raise HTTPException(status_code=404, detail=_("画册不存在", _lang_var.get()))
...
def add_trust(user_id: str, data: TrustAddRequest, current_user_id: str = Depends(require_user_id)):
    # 用户隔离：只能操作自己的信任网络
...
def remove_trust(
    user_id: str,
    trusted_user_id: str = Query(..., description="被信任的用户ID"),
...
def brochure_preview(brochure_id: str, request: Request, visitor_name: str = ""):
    """画册预览页 — 访问即记录访客。"""
    ...
    db_log_visitor(bid, visitor_ip=visitor_ip, visitor_name=visitor_name, source=source)
...
def sync_chainke(current_user_id: str = Depends(require_user_id)):
    ...
    将信任网络数据推送到链客宝AI，并从链客宝AI拉取推荐匹配。
    链客宝AI不可达时自动 fallback 到本地匹配引擎。
    ...
    if not bridge:
        return {
            "code": 0,
            "message": _("链客宝AI桥接模块未加载，同步跳过", _lang_var.get()),
            "data": {
                "trust_pushed": 0,
                "matches_pulled": 0,
                "source": "none",
    ...
    logger.info(
        "链客宝AI同步完成: user=%s, trust_pushed=%d, matches_pulled=%d, source=%s",
        current_user_id,
        trust_pushed,
        matches_pulled,
        source,
    ...
    for idx, item in enumerate(data.users):
        try:
            ...
            brochure = {
                ...
                "email": "",
                "wechat": "",
                "bio": f"{item.name} · {item.company_name} · {item.title} · {item.industry}"
                if item.industry
                else f"{item.name} · {item.company_name} · {item.title}",
                "tags": [{"tag": item.industry, "tag_type": "provide", "weight": 0.8, "source": "import"}]
                if item.industry
                else [],
                "trust_network": [],
                "created_at": now,
